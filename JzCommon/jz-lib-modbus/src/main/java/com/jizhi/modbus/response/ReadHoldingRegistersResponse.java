...
@Slf4j
public class ReadHoldingRegistersResponse extends BaseModbusResponse<int[]> {

    private byte[] rawData;
    private int[] responseData;

    public ReadHoldingRegistersResponse(byte[] pdu, int unitId) throws ModbusException {
        super(FunctionCodeConstants.ReadCoils, unitId);
        rawData = pdu;
        parseData(pdu);
    }

    public ReadHoldingRegistersResponse(byte[] pdu, int transactionId, int unitId) throws ModbusException {
    ...
    private void parseData(byte[] pdu) throws ModbusException {
        if (functionCode == (FunctionCodeConstants.ReadHoldingRegisters | 0x80)) {
        ...
        for (int i = 0; i < responseData.length; i++) {
            ...
            log.debug("保持寄存器位{}值={}", i, responseData[i]);
        }
    }
