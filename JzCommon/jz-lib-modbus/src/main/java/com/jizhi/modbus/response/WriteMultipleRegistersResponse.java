...
public class WriteMultipleRegistersResponse extends BaseModbusResponse<int[]> {

    private byte[] rawData;
    private int[] responseData;

    public WriteMultipleRegistersResponse(byte[] pdu, int unitId) throws ModbusException {
        super(FunctionCodeConstants.ReadCoils, unitId);
        rawData = pdu;
        parseData(pdu);
    }

    public WriteMultipleRegistersResponse(byte[] pdu, int transactionId, int unitId) throws ModbusException {
    ...
    private void parseData(byte[] pdu) throws ModbusException {
        if (functionCode == (FunctionCodeConstants.WriteMultipleRegisters | 0x80)) {
